...
# selected together for each simulated MPA for finwhale conservation.
...
## filter for non-decision makers fw

df <- df %>% filter(org_type!="Regulation and management")
...
plot(g, layout=layout_randomly, vertex.size=10, vertex.label=NA)

# add some color
## for fw
## all orgs
#colrs <- c('#d8b365','#ffbb00','#af8dc3','#4bb349','#67a9cf','#d9ef8b')
##non-decision makers
colrs <- c('#af8dc3','#67a9cf')
# corresponding: brown, orange, violet, dark gren, blue, light green
# "Algeria","France", "Italy","Monaco","Spain","Tunisia")
...
# # legend finwhale non-decision makers
legend(x=-1.5, y=-1.1, c("Italy","Spain"), pch=21,
       col=colrs, pt.bg=colrs, pt.cex=1, cex=.3, bty="n", ncol=1)


## add network centralization value to the graph (rounded at 2nd decimal)
c_value <- paste("Centralization score = ", round(b$centralization, digits = 2))

...
selected_csv <- paste0(input_dir, "/traffic_admins_FW_each-month.csv")



# import data
df <- read.csv(selected_csv, sep=';')
...
stak <- df %>%
  group_by(ID2, Org_nam, Org_type, Sector, Country) %>%
  summarise(n = n())
...
# add some color
## for fw
colrs <- c('#d8b365','#ffbb00','#800000','#af8dc3','#4bb349','#9AFEFF','#67a9cf','#d9ef8b')
# corresponding: brown, orange, violet, dark gren, blue, light green
# "Algeria","France","Gibraltar", "Italy","Monaco","Morocco","Spain","Tunisia")
...
# legend finwhale
legend(x=-1.5, y=-1.1, c("Algeria","France", "Italy","Monaco","Spain","Tunisia"), pch=21,
